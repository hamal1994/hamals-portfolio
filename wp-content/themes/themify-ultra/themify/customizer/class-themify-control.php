...
class Themify_Control extends WP_Customize_Control {
	...
	function render_fonts($values, $args = array()) {
			...
			<?php if ($args['show_family']) : ?>
				...
				<div class="custom-select themify_combobox font-family-select <?php if (!$args['show_size']) echo 'font-family-select-no-size' ?>">
					<?php if (empty(self::$webfonts)): ?>
						...
						?>
						<input id="themify_fonts_hidden" type="hidden" value="<?php echo esc_attr(wp_json_encode(array('google' => array_values(self::$googlefonts), 'fonts' => array_values(self::$webfonts)))); ?>" />
					<?php endif; ?>
				...
				<div class="custom-select themify-font-weight">
					<select class="font_weight_select">
						...
						<?php foreach ($weights as $k=>$w) : ?>
							<option <?php if($font_family && isset(self::$googlefonts[$font_family]) && !in_array($k,self::$googlefonts[$font_family]['variant'])):?>style="display:none;"<?php endif;?> value="<?php echo $k; ?>" <?php selected($k, $weight); ?>><?php echo $w; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
		...
		<?php if ($args['show_letterspacing']) : ?>
			<div class="themify-customizer-brick">
				<!-- LETTER SPACING -->
				<input type="text" class="letter_spacing" value="<?php echo esc_attr(empty($letter_spacing) ? '' : $letter_spacing ); ?>" />
				<div class="custom-select">
