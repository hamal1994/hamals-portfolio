...
class Themify_Builder_Updater {
	...
	public function __construct( $name, $version, $slug ) {
		...
		if( isset( $_GET['page'] ) && ! isset( $_GET['action'] ) && ( $_GET['page'] == 'themify-builder' || $_GET['page'] == 'themify' ) ) {
			...
			add_action( 'admin_notices', array( $this, 'new_version_available' ), 3 );
		}
...
log</a> for details.</p>', 'themify'),
	...
	public function new_version_available() {
		...
		if ( version_compare( $this->version, $remote_version, '<' ) ) {
			printf( __( '<div class="notifications">
				<p class="update-nag">%s version %s is now available. Go to 
				<a href="%s">%s settings</a> page to update.</p></div>', 'themify' ),
					$this->nicename,
					$remote_version,
					esc_url( add_query_arg( 'page', $this->name, admin_url( 'admin.php' ) ) ),
					$this->nicename
			);
		}
